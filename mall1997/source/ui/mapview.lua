...
local function tile(a, code, x, y)
  ...
  elseif code == A.CAB then
    black(); gfx.fillRect(px + 1, py - 6, T - 2, T + 6)
    white(); gfx.fillRect(px + 3, py - 3, T - 6, 7)
    gfx.fillRect(px + 4, py + 7, 2, 2); gfx.fillRect(px + 9, py + 7, 3, 3)
...
local function fitText(s, w)
  while #s > 1 and Gfx.textW(s, true) > w do s = s:sub(1, #s - 1) end
  return s
end
...
local function drawSign(a, o)
  ...
  if o.top == nil and a.id == "fc" then by = py + 4 end
  black(); gfx.fillRect(px + 2, by, w - 4, 20)
  white(); gfx.drawRect(px + 3, by + 1, w - 6, 18)
  Gfx.text(fitText(label, w - 10), px + w // 2, by + 2, { white = true, align = "center", bold = true })
...
function MapView.render(a)
  ...
  for _, o in ipairs(a.objs) do
    ...
    elseif o.kind == "bus" then
      black(); gfx.fillRect(px, py - 12, 3, 28); gfx.fillRect(px - 4, py - 16, 22, 8)
      Gfx.text("BUS", px + 7, py - 17, { white = true, align = "center" })
    ...
    if px + w >= x0 and px <= x1 and py + h >= y0 and py <= y1 then
      if o.kind == "sign" and o.store and not o.anchorEnd then
        ...
        if s and s.open then
          ...
          if NEON_TYPES[s.type] and open then
            local by = o.top and (py + T + 2) or (py + T + 4)
            if a.id == "fc" then by = py + 4 end
            -- buzzing neon frame
            gfx.setPattern(Gfx.NEON[1 + ((f // 3 + s.id) % #Gfx.NEON)])
            gfx.drawRect(px + 1, by - 1, w - 2, 22); gfx.drawRect(px, by - 2, w, 24)
            black()
            if ((f + s.id * 13) % 151) < 4 then Gfx.fill(px + 3, by + 1, w - 6, 18, "darker") end
          end
      ...
      elseif o.kind == "cab" then
        ...
        if m and m.broken then
          Gfx.fill(px + 3, py - 3, w - 6, 7, "white")
          Gfx.line(px + 3, py - 3, px + w - 4, py + 3)
        else
          gfx.setPattern(Gfx.NEON[1 + ((f // 3 + o.machine) % #Gfx.NEON)])
          gfx.fillRect(px + 3, py - 3, w - 6, 7)
          black()
        end
      elseif o.kind == "atrium" then
